...
using TestApp.Models.TokenAuth;
...
{
    public abstract class TestAppWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static TestAppWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(TestAppWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<TestAppDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<TestAppDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<TestAppDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<TestAppDbContext, Task<T>> func)
        {
            T result;
